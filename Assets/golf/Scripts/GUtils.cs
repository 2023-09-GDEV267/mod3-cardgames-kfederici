...
public class GEasing
{
    ...
    static public Dictionary<string, GEasingCachedCurve> Gcache;
    ...
    static public float Ease(float u, params string[] curveParams)
    {
        ...
        if (Gcache == null)
        {
            Gcache = new Dictionary<string, GEasingCachedCurve>();
        }
        ...
        foreach (string curve in curveParams)
        {
            ...
            if (!Gcache.ContainsKey(curve))
            {
                // If not, parse and cache it
                EaseParse(curve);
            }
            // Call the cached curve
            u2 = EaseP(u2, Gcache[curve]);
        }
        return (u2);
        /*	
    ...
    {
        GEasingCachedCurve ecc = new GEasingCachedCurve();
        // It's possible to pass in several comma-separated curves
